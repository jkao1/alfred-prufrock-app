function tree(){
	var svgW=400, svgH =460, vRad=35, tree={cx:200, cy:75, w:40, h:90};
	tree.vis={v:0, l:'Go!', p:{x:tree.cx, y:tree.cy},c:[]};
	tree.size=1;
	tree.glabels =[];
	tree.incMatx =[];
	...
	tree.addLeaf = function(_, entityWord){
		function addLeaf(t){
			if(t.v==_){ t.c.push({v:tree.size++, l:entityWord, p:{},c:[]}); return; }
			t.c.forEach(addLeaf);
		}
		addLeaf(tree.vis);
		reposition(tree.vis);
    ...
    */
		d3.select("#theTreeDiv").append("svg").attr("width", svgW).attr("height", svgH).attr('id','treesvg');
